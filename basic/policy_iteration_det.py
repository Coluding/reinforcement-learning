...
if __name__ == '__main__':
    ...
    while True:
        ...
        for s in grid.actions.keys():
            ...
            # looping through all actions and states
            # Look whether there is a reward connected to moving from s to s2 through s2 and if thise movement is possible
            # using the transition probability. If its possible and there is a reward and value v is updated for the current action
            # We are updating v for all states that follow through the action a from state s
            # (in the deterministic case this is only ones state)
            for a in ACTION_SPACE:
                v = 0
                for s2 in grid.all_states():
                    # reward is a function of (s, a, s'), 0 if not specified
